...
const PricingCard: React.FC<PricingCardProps> = ({ tier, price, features, isFeatured = false }) => {
  return (
    <div className={`relative p-6 md:p-8 rounded-2xl glass-morphism border ${isFeatured ? 'border-cyan-400 shadow-2xl shadow-cyan-500/20' : 'border-gray-700/50'}`}>
      {isFeatured && (
      ...
      <h3 className="font-orbitron text-xl md:text-2xl font-bold text-glow">{tier} Tier</h3>
      <p className="text-3xl md:text-4xl font-bold mt-3 md:mt-4">
        {price}
        {tier !== 'Free' && <span className="text-base md:text-lg font-normal text-gray-400">/ month</span>}
      </p>
      <ul className="mt-6 md:mt-8 space-y-2 md:space-y-3 text-gray-300 text-sm md:text-base">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center">
            <svg className="w-4 h-4 md:w-5 md:h-5 text-cyan-400 mr-2 md:mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
            {feature}
          </li>
        ))}
      </ul>
