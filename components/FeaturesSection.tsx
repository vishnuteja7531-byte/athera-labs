...
const FeatureCard: React.FC<{ title: string; index: number }> = ({ title, index }) => {
  return (
    <div 
      className="group p-4 rounded-xl glass-morphism transition-all duration-500 ease-out transform-gpu"
      style={{
    ...
    >
      <div className="relative w-full h-40 flex items-center justify-center text-center p-4 rounded-lg transition-all duration-500 ease-out group-hover:scale-105 group-hover:-translate-z-4 bg-black/20 transform-gpu group-hover:[transform:translateZ(20px)_rotateX(5deg)]">
        ...
        <h3 className="font-orbitron text-lg font-bold text-white group-hover:text-glow">{title}</h3>
      </div>
    </div>
  );
};
